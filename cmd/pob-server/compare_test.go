...
func compareHarness(
	...
	t.Helper()
	pool, _ := captureMockPool(t, []string{calcResponseA, calcResponseB})
	pool.maxSize = 1
...
func TestCompareN3HappyPath(t *testing.T) {
	...
	mustString := func(i int) string {
		s, ok := resp.Builds[i].Character["class"].(string)
		if !ok {
			t.Fatalf("build[%d].class not a string: %T", i, resp.Builds[i].Character["class"])
		}
		return s
	}
	classes := []string{mustString(0), mustString(1), mustString(2)}
