...
/**
 * Call the PoB calc service with a build code and return structured build data.
 * Returns a graceful error if the service is unreachable.
 */
export async function pobCalc(
  pobUrl: string | undefined,
  pobApiKey: string | undefined,
  buildCode: string,
): Promise<ToolResult> {
  if (!pobUrl) {
    return errorResult(
