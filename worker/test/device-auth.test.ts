...
describe("Device Token Authentication", () => {
  ...
  it("updates last_push_at on successful auth", async () => {
    ...
    const before = await env.DB.prepare("SELECT last_push_at FROM devices WHERE device_uuid = ?")
    ...
    const after = await env.DB.prepare("SELECT last_push_at FROM devices WHERE device_uuid = ?")
      .bind(device.device_uuid)
      .first<{ last_push_at: string | null }>();
