...
import { rulesSearchModule } from "../../plugins/magic/reference/rules-search";
...
describe("rules_search native module", () => {
  ...
  it("is registered as a native module for mtga", () => {
    const module_ = getNativeModule("magic", "rules_search");
  ...
  async function seedInteractions(): Promise<void> {
    await env.DB.batch([
      env.DB.prepare(
        `INSERT INTO magic_interactions (id, title, mechanics, card_names, rule_numbers, breakdown, common_error)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
      ).bind(
        1,
        "Blood Moon + Sagas",
