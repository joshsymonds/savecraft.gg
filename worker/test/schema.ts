// Consolidated D1 schema for test and eval environments.
//
// This is the single schema source for workerd-external consumers: vitest's
// setup.ts applies it before tests, and scripts/eval-sidecar.ts applies it to
// a Miniflare D1 for the fair-eval harness. Keep it in sync with migrations/.
export const SCHEMA_STATEMENTS = [
  ...
  `CREATE INDEX IF NOT EXISTS idx_saves_source_refresh ON saves(last_source_uuid, last_refresh_at)`,
  ...
  // EDHREC tier metadata + per-tier average decks (migration 0052)
