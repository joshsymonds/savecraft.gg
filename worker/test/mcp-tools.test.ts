...
describe("MCP Tools", () => {
  ...
  describe("getInfo", () => {
    ...
    it("returns all supported games for category='games'", async () => {
      ...
      // All embedded manifests returned, sorted alphabetically by name
    ...
    it("games category returns all embedded manifests", async () => {
      ...
      // All embedded manifests are always present
      expect(data.games).toHaveLength(12);
  ...
  it("returns error when neither build nor build_id provided", async () => {
    ...
    expect(result).toEqual({
      ...
      content: expect.stringContaining(
        "provide character (your connected PoE character), a build URL, or a build_id",
      ),
    });
  });
