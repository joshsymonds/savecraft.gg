...
describe("MTGA rules D1 schema", () => {
  ...
  // ── magic_rules table + FTS5 ────────────────────────────────

  it("inserts and retrieves rules", async () => {
    await env.DB.prepare(
      "INSERT INTO magic_rules (number, text, example, see_also) VALUES (?, ?, ?, ?)",
    )
      .bind("702.2", "Deathtouch is a static ability.", null, null)
      .run();

    const row = await env.DB.prepare("SELECT * FROM magic_rules WHERE number = ?")
      .bind("702.2")
  ...
  it("FTS5 keyword search returns ranked results", async () => {
    ...
    await env.DB.batch([
      ...
      env.DB.prepare("INSERT INTO magic_rules_fts (number, text, example) VALUES (?, ?, ?)").bind(
        "702.2",
    ...
    const results = await env.DB.prepare(
      `SELECT number, rank FROM magic_rules_fts WHERE magic_rules_fts MATCH ? ORDER BY rank LIMIT 10`,
    )
      .bind("deathtouch")
  ...
  it("FTS5 porter stemming matches inflected forms", async () => {
    ...
    const results = await env.DB.prepare(
      `SELECT number FROM magic_rules_fts WHERE magic_rules_fts MATCH ? LIMIT 10`,
    )
      .bind("destroy")
      .all<{ number: string }>();
