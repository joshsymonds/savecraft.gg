...
A web-only PR skips Go and Worker checks entirely. Deploy jobs run only on `main` push and only if the relevant paths changed. Skipped test jobs don't block deploys.
