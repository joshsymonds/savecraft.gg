...
The **daemon** instead fetches a CI-signed aggregate, `/plugins/manifest.json` + `/plugins/manifest.json.sig`, from the **server origin** — the same Worker/origin that serves the plugin binaries, so all plugin delivery lives on one origin (the Worker serves the signed bytes verbatim from R2; it is not a trust anchor). A CI conclusion job in `deploy-plugin.yml` rebuilds the whole aggregate from every plugin's current published per-plugin `manifest.json` in R2 (so a partial deploy still yields a complete manifest), signs the literal bytes with the release key, and uploads both. The daemon pins every URL to the server origin, verifies the detached signature against the embedded key, and only then parses it (`internal/pluginmgr/httpregistry.go`, reusing `internal/manifest.VerifyAndParse` + `RequirePinnedHTTPS`). A missing/invalid signature is a hard error. (Daemon **binary** self-update is separate and still uses the install origin — see `docs/daemon.md`.)
...
**Polling:** Daemon fetches the signed `/plugins/manifest.json` (+`.sig`) from the server origin on startup and every 24 hours. Response is a JSON object mapping game IDs to plugin metadata (version, sha256, url, plus all `plugin.toml` fields). If the plugin has a reference module, the manifest includes a `reference` field with its own sha256, url, and module list. Daemon compares local versions, downloads updates as needed.

**Anti-rollback:** The daemon refuses a signed-manifest plugin whose version is strictly **older** than the version already cached for that game (first install, equal/newer, or an identical re-tagged artifact are allowed; an empty/garbage version compares as not-newer and is refused — fail closed). This is a deliberately **lightweight** control (a `version.IsNewer` check against the cache anchor): it defends the realistic channel/replay/MITM adversary, where the local cache is intact, and is matched to the sandboxed risk of a rolled-back parser. It is **not** integrity-protected persisted state — a local-FS attacker who can rewrite the cache could also just drop an old signed `.wasm` directly, so heavyweight keyring/TPM state would defend a sliver no realistic adversary occupies while adding a fragile cross-OS dependency. Sandboxing and unconditional signature verification already cap the blast radius. (The daemon **binary** has a stronger guarantee — its anti-rollback floor is the compiled-in version baked into the signed binary.)
