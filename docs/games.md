...
The MTGA plugin extracts game state from the detailed output log that MTGA writes during play. It produces a `player_summary` with display name, rank in constructed and limited, currencies (gold, gems, wildcards, boosters, vault progress), deck names, and indexes into match and game log data. Note that MTGA does not log the player's card collection to Player.log — ownership queries ("what cards do I own?") cannot be answered from save data; use the `card_search` reference module for general card lookups. Each deck the player owns becomes a `deck:{name}` section with full card lists for main deck, sideboard, and command zone. Completed matches appear as `match:{id}` sections with opponent info, rank, and cards seen. Individual games within matches become `game:{id}` sections with turn-by-turn play logs. Draft sessions produce a `draft_history` section with every pick, the pack contents, and the player's pool at each selection. The `match:`/`game:` sections and the `player_summary` indexes into them only cover matches recorded in the current Player.log — MTGA truncates this log on client restart, so older matches roll off. Cumulative match history (summary stats and trends, not turn-by-turn logs) is available via the `match_stats` reference module.

The player's screen name comes from MTGA's match-connection log event, not client login — it's only written to Player.log once the player queues into a match. A session where the player never queues shows a generic name; the first match of a watched session fills it in automatically, and it persists across future sessions once learned.
