...
|------|-----------------|-------------------|-----------|
| D2R Character | `character` (no overview match — needs fix) | None (sections are small) | `plugins/d2r/parser/main.go` |
...
| SDV | None (falls back to `bundles` — needs fix) | None | `plugins/sdv/parser/main.go` |
