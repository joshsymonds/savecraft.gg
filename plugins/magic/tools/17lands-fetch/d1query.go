...
// fetchCardRoles queries D1 for card name → set of roles mapping from magic_card_roles.
...
func fetchCardRoles(accountID, databaseID, apiToken string) (map[string]map[string]bool, error) {
	rows, err := cfapi.QueryD1(accountID, databaseID, apiToken,
		"SELECT front_face_name, role FROM magic_card_roles WHERE front_face_name != ''")
	if err != nil {
		return nil, fmt.Errorf("querying magic_card_roles: %w", err)
	}
