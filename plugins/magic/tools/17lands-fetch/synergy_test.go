...
func TestBuildSetSynergySQL(t *testing.T) {
	...
	if !strings.Contains(sql, "DELETE FROM magic_draft_synergies WHERE set_code = 'DSK';") {
		t.Error("SQL should contain per-set DELETE for synergies")
	}
	if !strings.Contains(sql, "DELETE FROM magic_draft_archetype_curves WHERE set_code = 'DSK';") {
		t.Error("SQL should contain per-set DELETE for curves")
	}
	if !strings.Contains(sql, "DELETE FROM magic_draft_deck_stats WHERE set_code = 'DSK';") {
		t.Error("SQL should contain per-set DELETE for deck stats")
	}
	// Must NOT contain global DELETEs
	if strings.Contains(sql, "DELETE FROM magic_draft_synergies;") {
		t.Error("SQL should NOT contain global DELETE (no WHERE clause)")
	}

	if !strings.Contains(sql, "INSERT INTO magic_draft_synergies") {
	...
	synergyCount := strings.Count(sql, "INSERT INTO magic_draft_synergies")
	if synergyCount != 2 {
		t.Errorf("expected 2 synergy INSERTs, got %d", synergyCount)
	}
	curveCount := strings.Count(sql, "INSERT INTO magic_draft_archetype_curves")
	if curveCount != 1 {
...
func TestBuildSynergyImportSQL_RoleTargets(t *testing.T) {
	...
	if !strings.Contains(sql, "DELETE FROM magic_draft_role_targets WHERE set_code = 'DSK';") {
		t.Error("SQL should contain per-set DELETE for role targets")
	}

	rtCount := strings.Count(sql, "INSERT INTO magic_draft_role_targets")
	if rtCount != 2 {
...
func TestBuildSynergyImportSQL_DeckStats(t *testing.T) {
	...
	dsCount := strings.Count(sql, "INSERT INTO magic_draft_deck_stats")
	if dsCount != 1 {
...
func TestBuildSetRatingsSQL(t *testing.T) {
	...
	if !strings.Contains(sql, "DELETE FROM magic_draft_ratings_fts WHERE set_code = 'DSK';") {
		t.Error("SQL should contain per-set DELETE for ratings_fts")
	}
	if !strings.Contains(sql, "DELETE FROM magic_draft_ratings WHERE set_code = 'DSK';") {
		t.Error("SQL should contain per-set DELETE for ratings")
	}
	// Must NOT contain global DELETEs
	if strings.Contains(sql, "DELETE FROM magic_draft_ratings;") {
		t.Error("SQL should NOT contain global DELETE (no WHERE clause)")
	}

	if !strings.Contains(sql, "INSERT INTO magic_draft_set_stats") {
		t.Error("SQL should contain INSERT INTO magic_draft_set_stats")
	}
	if !strings.Contains(sql, "Gloomlake Verge") {
		t.Error("SQL should contain card name")
	}
	if !strings.Contains(sql, "INSERT INTO magic_draft_archetype_stats") {
		t.Error("SQL should contain INSERT INTO magic_draft_archetype_stats")
	}
	...
	overallCount := strings.Count(sql, "INSERT INTO magic_draft_ratings (")
	if overallCount != 2 {
		t.Errorf("expected 2 overall rating INSERTs, got %d", overallCount)
	}
	colorCount := strings.Count(sql, "INSERT INTO magic_draft_archetype_stats")
	if colorCount != 1 {
		t.Errorf("expected 1 color stat INSERT, got %d", colorCount)
	}
	ftsCount := strings.Count(sql, "INSERT INTO magic_draft_ratings_fts")
	if ftsCount != 2 {
...
func TestBuildSetRatingsSQL_EmptyCards(t *testing.T) {
	...
	if strings.Contains(sql, "INSERT INTO magic_draft_ratings (") {
