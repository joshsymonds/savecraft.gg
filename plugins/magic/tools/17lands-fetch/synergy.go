...
func buildSetSynergySQL(r synergyDataResult) string {
	...
	fmt.Fprintf(&b, "DELETE FROM magic_draft_synergies WHERE set_code = %s;\n", sc)
	fmt.Fprintf(&b, "DELETE FROM magic_draft_archetype_curves WHERE set_code = %s;\n", sc)
	fmt.Fprintf(&b, "DELETE FROM magic_draft_role_targets WHERE set_code = %s;\n", sc)
	fmt.Fprintf(&b, "DELETE FROM magic_draft_deck_stats WHERE set_code = %s;\n", sc)
	fmt.Fprintf(&b, "DELETE FROM magic_draft_calibration WHERE set_code = %s;\n", sc)

	for _, s := range r.Synergies {
		fmt.Fprintf(&b, "INSERT INTO magic_draft_synergies (set_code, card_a, card_b, synergy_delta, games_together) VALUES (%s, %s, %s, %g, %d);\n",
			sc, q(s.CardA), q(s.CardB), s.SynergyDelta, s.GamesTogether)
	}
	for _, c := range r.Curves {
		fmt.Fprintf(&b, "INSERT INTO magic_draft_archetype_curves (set_code, archetype, cmc, avg_count, total_decks) VALUES (%s, %s, %d, %g, %d);\n",
			sc, q(c.Archetype), c.CMC, c.AvgCount, c.TotalDecks)
	}
	for _, rt := range r.RoleTargets {
		fmt.Fprintf(&b, "INSERT INTO magic_draft_role_targets (set_code, archetype, role, avg_count, total_decks) VALUES (%s, %s, %s, %g, %d);\n",
			sc, q(rt.Archetype), q(rt.Role), rt.AvgCount, rt.TotalDecks)
	}
	for _, ds := range r.DeckStats {
		fmt.Fprintf(&b, "INSERT INTO magic_draft_deck_stats (set_code, archetype, avg_lands, avg_creatures, avg_noncreatures, avg_fixing, splash_rate, splash_avg_sources, splash_winrate, nonsplash_winrate, total_decks) VALUES (%s, %s, %g, %g, %g, %g, %g, %g, %g, %g, %d);\n",
	...
	for _, cal := range r.Calibration {
		fmt.Fprintf(&b, "INSERT INTO magic_draft_calibration (set_code, axis, center, steepness) VALUES (%s, %s, %g, %g);\n",
...
func buildSetRatingsSQL(sr setResult) string {
	...
	// Per-set DELETEs.
	fmt.Fprintf(&b, "DELETE FROM magic_draft_ratings_fts WHERE set_code = %s;\n", sc)
	fmt.Fprintf(&b, "DELETE FROM magic_draft_archetype_stats WHERE set_code = %s;\n", sc)
	fmt.Fprintf(&b, "DELETE FROM magic_draft_ratings WHERE set_code = %s;\n", sc)
	fmt.Fprintf(&b, "DELETE FROM magic_draft_set_stats WHERE set_code = %s;\n", sc)

	fmt.Fprintf(&b, "INSERT INTO magic_draft_set_stats (set_code, format, total_games, card_count, avg_gihwr) VALUES (%s, 'PremierDraft', %d, %d, %g);\n",
		sc, sr.TotalGames, sr.CardCount, round4(sr.AvgGIHWR))

	for _, c := range sr.Cards {
		o := c.Overall
		fmt.Fprintf(&b, "INSERT INTO magic_draft_ratings (set_code, card_name, games_in_hand, games_played, games_not_seen, gihwr, ohwr, gdwr, gnswr, iwd, alsa, ata, ata_stddev) VALUES (%s, %s, %d, %d, %d, %g, %g, %g, %g, %g, %g, %g, %g);\n",
			sc, q(c.Name),
		...
		fmt.Fprintf(&b, "INSERT INTO magic_draft_ratings_fts (set_code, card_name) VALUES (%s, %s);\n",
			sc, q(c.Name))
		for cp, s := range c.ByColor {
			fmt.Fprintf(&b, "INSERT INTO magic_draft_archetype_stats (set_code, card_name, archetype, games_in_hand, games_played, games_not_seen, gihwr, ohwr, gdwr, gnswr, iwd, alsa, ata, ata_stddev) VALUES (%s, %s, %s, %d, %d, %d, %g, %g, %g, %g, %g, %g, %g, %g);\n",
				sc, q(c.Name), q(cp),
