...
func buildImportSQL(rules []Rule) string {
	...
	// Clear existing data
	b.WriteString("DELETE FROM magic_rules_fts;\n")
	b.WriteString("DELETE FROM magic_rules;\n")
	...
	for _, r := range rules {
		...
		fmt.Fprintf(&b, "INSERT INTO magic_rules (number, text, example, see_also) VALUES (%s, %s, %s, %s);\n",
			q(r.Number), q(r.Text), example, seeAlso)
		fmt.Fprintf(&b, "INSERT INTO magic_rules_fts (number, text, example) VALUES (%s, %s, %s);\n",
...
func buildInteractionsImportSQL(interactions []Interaction) string {
	...
	b.WriteString("DELETE FROM magic_interactions_fts;\n")
	b.WriteString("DELETE FROM magic_interactions;\n")

	for i, inter := range interactions {
		id := i + 1
		fmt.Fprintf(&b, "INSERT INTO magic_interactions (id, title, mechanics, card_names, rule_numbers, breakdown, common_error) VALUES (%d, %s, %s, %s, %s, %s, %s);\n",
			id, q(inter.Title), q(inter.Mechanics), q(inter.CardNames), q(inter.RuleNumbers), q(inter.Breakdown), q(inter.CommonError))
		fmt.Fprintf(&b, "INSERT INTO magic_interactions_fts (id, title, mechanics, card_names, breakdown) VALUES (%d, %s, %s, %s, %s);\n",
