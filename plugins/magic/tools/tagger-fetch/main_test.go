...
func TestBuildSetRolesSQL(t *testing.T) {
	...
	if !strings.Contains(sql, "DELETE FROM magic_card_roles WHERE set_code = 'DSK';") {
		t.Error("SQL should contain per-set DELETE")
	}
	// Must NOT contain global DELETE
	if strings.Contains(sql, "DELETE FROM magic_card_roles;") {
		t.Error("SQL should NOT contain global DELETE")
	}

	insertCount := strings.Count(sql, "INSERT INTO magic_card_roles")
	if insertCount != 3 {
