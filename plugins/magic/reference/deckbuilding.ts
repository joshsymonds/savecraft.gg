 ...
 *      per-set data from magic_draft_deck_stats + scoring primitives.
...
async function inferSet(
  ...
  for (let i = 0; i < cardNames.length; i += META_BATCH_SIZE) {
    ...
    const rows = await db
      .prepare(
        `SELECT set_code, COUNT(*) as matches FROM magic_draft_ratings WHERE card_name IN (${ph}) GROUP BY set_code`,
      )
      .bind(...chunk)
...
async function buildArchetypeInfo(
  ...
  const allDeckStats = await db
    .prepare(
      `SELECT archetype, total_decks FROM magic_draft_deck_stats WHERE set_code = ?1`,
    )
    .bind(setCode)
...
async function healthCheck(
  ...
  const [deckStatsResult, curveResult, roleResult, cardRolesResult] =
    await Promise.all([
      db
        .prepare(
          `SELECT * FROM magic_draft_deck_stats WHERE set_code = ?1 AND archetype = ?2`,
        )
        .bind(setCode, primaryArchetype)
        .all<DeckStatsRow>(),
      db
        .prepare(
          `SELECT cmc, avg_count FROM magic_draft_archetype_curves WHERE set_code = ?1 AND archetype = ?2`,
        )
        .bind(setCode, primaryArchetype)
        .all<CurveDbRow>(),
      db
        .prepare(
          `SELECT role, avg_count FROM magic_draft_role_targets WHERE set_code = ?1 AND archetype = ?2`,
        )
        .bind(setCode, primaryArchetype)
        .all<RoleTargetRow>(),
      db
        .prepare(
          `SELECT front_face_name, role FROM magic_card_roles WHERE set_code = ?1`,
        )
        .bind(setCode)
        .all<CardRoleRow>(),
    ]);
...
async function computeAlternatives(
  ...
  for (let i = 0; i < spellNames.length; i += META_BATCH_SIZE) {
    ...
    const rows = await db
      .prepare(
        `SELECT card_name, archetype, gihwr FROM magic_draft_archetype_stats WHERE set_code = ?1 AND card_name IN (${cardPH})`,
      )
      .bind(setCode, ...chunk)
...
async function cutAdvisor(
  ...
  for (let i = 0; i < spellNames.length; i += META_BATCH_SIZE) {
    ...
    const rows = await db
      .prepare(
        `SELECT card_name, gihwr FROM magic_draft_ratings WHERE set_code = ?1 AND card_name IN (${ph})`,
      )
      .bind(setCode, ...chunk)
  ...
  for (let i = 0; i < spellNames.length; i += META_BATCH_SIZE) {
    ...
    const rows = await db
      .prepare(
        `SELECT card_a, card_b, synergy_delta FROM magic_draft_synergies WHERE set_code = ?1 AND card_a IN (${ph})`,
      )
      .bind(setCode, ...chunk)
      .all<SynergyDbRow>();
    for (const row of rows.results) {
...
async function commanderHealthCheck(
  db: D1Database,
  deck: DeckEntry[],
  ...
  if (!commanderName) {
    return {
      ...
      data: {
        ...
        format: "commander",
        errors: [
          "Commander format requires a `commander` parameter (the commander card's name).",
        ],
      },
    };
  }

  // Look up the commander card
  const commanderRow = await db
    .prepare(
      `SELECT front_face_name AS name, color_identity, legalities, type_line
       FROM magic_cards
       WHERE front_face_name COLLATE NOCASE = ?
       LIMIT 1`,
    )
    .bind(commanderName)
    .first<CommanderCardRow>();
  ...
  if (!commanderRow) {
    return {
      ...
      data: {
        mode: "constructed",
        format: "commander",
        errors: [`Commander card not found: "${commanderName}".`],
      },
    };
  }

  const commanderColorIdentity = safeParseJSON<string[]>(
    commanderRow.color_identity,
    [],
  );
  const commanderColorSet = new Set(commanderColorIdentity);

  // Fetch card metadata for every card in the deck
  const uniqueNames = [...new Set(deck.map((e) => e.name))];
  const cardMeta = new Map<string, CommanderCardRow>();
  for (let i = 0; i < uniqueNames.length; i += META_BATCH_SIZE) {
    ...
    const rows = await db
      ...
      .bind(...chunk)
      .all<CommanderCardRow>();
    for (const row of rows.results) {
      const key = row.name.toLowerCase();
      const existing = cardMeta.get(key);
      // Prefer rows with populated legalities (Art Series fallback).
      if (!existing || existing.legalities === "{}") {
        cardMeta.set(key, row);
      }
    }
  }

  const singletonViolations: { card_name: string; count: number }[] = [];
  const colorIdentityViolations: {
    card_name: string;
    card_colors: string[];
  }[] = [];
  const bannedCards: { card_name: string; status: string }[] = [];
  const unresolvedCards: string[] = [];

  for (const entry of deck) {
    const meta = cardMeta.get(entry.name.toLowerCase());
    if (!meta) {
    ...
    // Singleton: non-basics can appear only once, commander included.
    if (!isBasic && entry.count > 1) {
      singletonViolations.push({ card_name: meta.name, count: entry.count });
    }

    // Color identity: card's color identity must be subset of commander's.
    const cardColors = safeParseJSON<string[]>(meta.color_identity, []);
    const offColor = cardColors.filter((c) => !commanderColorSet.has(c));
    ...
    // Banned list
    try {
      const leg = JSON.parse(meta.legalities) as Record<string, string>;
      const status = leg.commander ?? "";
      if (status === "banned" || status === "restricted") {
        bannedCards.push({ card_name: meta.name, status });
      }
    } catch {
      // skip unparseable
    }
  ...
  // Deck size (commander + 99 others = 100)
  const deckSize = deck.reduce((sum, e) => sum + e.count, 0);
  if (deckSize !== 100) {
    errors.push(
      `Commander decks must contain exactly 100 cards (commander + 99). This deck has ${deckSize}.`,
    );
  }

  // Commander must be in the deck list
  const commanderInDeck = deck.some(
    (e) => e.name.toLowerCase() === commanderRow.name.toLowerCase(),
  );
  if (!commanderInDeck) {
    errors.push(
      `Commander "${commanderRow.name}" must appear in the deck list (count: 1).`,
    );
  }
...
async function constructedHealthCheck(
  ...
  for (let i = 0; i < allNames.length; i += META_BATCH_SIZE) {
    ...
    for (const row of rows.results) {
      ...
      if (format && row.legalities !== "{}") {
        try {
          const existingLeg = JSON.parse(existing.legalities) as Record<
            string,
            string
          >;
  ...
  const unresolvedLegality = allNames.filter(
    (n) => !cardData.has(n.toLowerCase()),
  );
...
export const deckbuildingModule: NativeReferenceModule = {
  ...
  parameters: {
    ...
    format: {
      type: "string",
      description:
        'Format for legality checking in constructed mode (e.g., "standard", "historic", "explorer", "commander"). Pass "commander" to get EDH-specific validation: singleton, color identity, banned list, 100-card size.',
    },
    commander: {
      type: "string",
      description:
        'Commander card name. Required when format="commander" — used to enforce color identity and singleton rules.',
    },
    sideboard: {
      type: "array",
      items: {
  ...
  async execute(
    ...
    if (query.mode === "constructed") {
      if (((query.format as string) ?? "").toLowerCase() === "commander") {
        return commanderHealthCheck(
          db,
          deck,
          query.commander as string | undefined,
        );
      }
