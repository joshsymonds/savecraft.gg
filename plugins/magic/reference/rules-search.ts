...
async function searchInteractionsByKeyword(
  ...
  const ftsResults = await db
    .prepare(
      `SELECT id FROM magic_interactions_fts WHERE magic_interactions_fts MATCH ?1 ORDER BY rank LIMIT ?2`,
    )
  ...
  const rows = await db
    .prepare(`SELECT * FROM magic_interactions WHERE id IN (${placeholders})`)
    .bind(...numericIds)
    .all<InteractionRow>();
...
async function searchInteractionsByRuleNumber(
  ...
  const rows = await db
    .prepare(`SELECT * FROM magic_interactions WHERE ${conditions} LIMIT ?`)
...
async function searchRulesByFts(
  ...
  const ftsResults = await db
    .prepare(
      `SELECT number FROM magic_rules_fts WHERE magic_rules_fts MATCH ?1 ORDER BY rank LIMIT ?2`,
    )
  ...
  const ruleRows = await db
    .prepare(`SELECT * FROM magic_rules WHERE number IN (${placeholders})`)
    .bind(...ids)
    .all<RuleRow>();
...
async function searchByRuleNumber(
  ...
  const rows = await db
    .prepare(
      `SELECT * FROM magic_rules WHERE number = ?1 OR (number LIKE ?2 AND length(number) = length(?1) + 1)`,
    )
  ...
  if (cappedRefs.length > 0) {
    ...
    const refRows = await db
      .prepare(
        `SELECT number, text FROM magic_rules WHERE number IN (${placeholders})`,
      )
      .bind(...cappedRefs)
      .all<RuleRow>();
