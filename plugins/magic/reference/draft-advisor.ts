...
interface PreloadedSetData {
  ...
  packSize: number;
  /** Deck stats per archetype from magic_draft_deck_stats. */
...
async function preloadSetData(
  ...
  const [
    ...
    db
      .prepare(
        `SELECT axis, center, steepness FROM magic_draft_calibration WHERE set_code = ?1`,
      )
      .bind(setCode)
      .all<CalibrationRow>(),
    db
    ...
    db
      .prepare(
        `SELECT archetype, cmc, avg_count, total_decks FROM magic_draft_archetype_curves WHERE set_code = ?1`,
      )
      .bind(setCode)
      .all<CurveDbRow>(),
    db
      ...
      .all<CardRoleRow>(),
    db
      .prepare(`SELECT * FROM magic_draft_ratings WHERE set_code = ?1`)
      .bind(setCode)
      .all<RatingRow>(),
    db
      .prepare(
        `SELECT set_code, card_name, archetype, games_in_hand, games_played, games_not_seen, gihwr, ohwr, gdwr, gnswr, iwd, alsa, ata FROM magic_draft_archetype_stats WHERE set_code = ?1`,
      )
      .bind(setCode)
      .all<RatingRow & { archetype: string }>(),
    db
      .prepare(
        `SELECT archetype, total_decks, splash_rate, splash_winrate, nonsplash_winrate FROM magic_draft_deck_stats WHERE set_code = ?1`,
      )
      .bind(setCode)
      .all<{
        archetype: string;
...
async function contextualPick(
  ...
  const overallPromise: Promise<{ results: RatingRow[] }> = preloaded
    ...
    : db
        .prepare(
          `SELECT * FROM magic_draft_ratings WHERE set_code = ?1 AND card_name IN (${placeholders(allCardNames.length, 2)})`,
        )
  ...
  const colorStatsPromise: Promise<{
    ...
    : db
        .prepare(
          `SELECT set_code, card_name, archetype, games_in_hand, games_played, games_not_seen, gihwr, ohwr, gdwr, gnswr, iwd, alsa, ata FROM magic_draft_archetype_stats WHERE set_code = ?1 AND card_name IN (${placeholders(packNames.length, 2)})`,
        )
  ...
  const synergyPromise =
    poolNames.length > 0 && packNames.length > 0
      ? (() => {
          ...
          return db
            .prepare(
              `SELECT card_a, card_b, synergy_delta, games_together FROM magic_draft_synergies WHERE set_code = ?1 AND card_a IN (${packPH}) AND card_b IN (${poolPH})`,
            )
  ...
  const rolePromise: Promise<{ results: CardRoleRow[] }> = preloaded
    ...
    : allCardNames.length > 0
      ? (() => {
          ...
          return db
            .prepare(
              `SELECT front_face_name, role FROM magic_card_roles WHERE set_code = ?1 AND front_face_name IN (${rolePH})`,
            )
            .bind(setCode, ...allCardNames)
            .all<CardRoleRow>();
        })()
    ...
    if (historyCards.size > 0) {
      ...
      if (preloaded) {
        ...
        const ataResult = await db
          .prepare(
            `SELECT card_name, ata, ata_stddev FROM magic_draft_ratings WHERE set_code = ?1 AND card_name IN (${histPH})`,
          )
    ...
    if (!setCode) {
      ...
      for (let i = 0; i < nameList.length; i += META_BATCH_SIZE) {
        ...
        const chunkResult = await env.DB.prepare(
          `SELECT set_code, COUNT(*) as matches FROM magic_draft_ratings WHERE card_name IN (${inferPH}) GROUP BY set_code`,
        )
          .bind(...chunk)
