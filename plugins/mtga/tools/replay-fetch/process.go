...
func buildReplaySQL(r *replayResult) string {
	...
	// Per-set DELETEs.
	fmt.Fprintf(&b, "DELETE FROM mtga_play_card_timing WHERE set_code = %s;\n", sc)
	fmt.Fprintf(&b, "DELETE FROM mtga_play_tempo WHERE set_code = %s;\n", sc)
	fmt.Fprintf(&b, "DELETE FROM mtga_play_combat WHERE set_code = %s;\n", sc)
	fmt.Fprintf(&b, "DELETE FROM mtga_play_mulligan WHERE set_code = %s;\n", sc)
	fmt.Fprintf(&b, "DELETE FROM mtga_play_turn_baselines WHERE set_code = %s;\n", sc)

	for _, ct := range r.cardTiming {
		fmt.Fprintf(&b, "INSERT INTO mtga_play_card_timing (set_code, card_name, archetype, turn_number, times_deployed, games_won, total_games) VALUES (%s, %s, %s, %d, %d, %d, %d);\n",
	...
	for _, t := range r.tempo {
		...
		fmt.Fprintf(&b, "INSERT INTO mtga_play_tempo (set_code, archetype, turn_number, on_play, mana_spent_bucket, games_won, total_games) VALUES (%s, %s, %d, %d, %d, %d, %d);\n",
	...
	for _, c := range r.combat {
		...
		fmt.Fprintf(&b, "INSERT INTO mtga_play_combat (set_code, attacker_name, turn_number, user_creatures_count, oppo_creatures_count, attacked, games_won, total_games) VALUES (%s, %s, %d, %d, %d, %d, %d, %d);\n",
	...
	for _, m := range r.mulligan {
		...
		fmt.Fprintf(&b, "INSERT INTO mtga_play_mulligan (set_code, archetype, on_play, land_count, nonland_cmc_bucket, num_mulligans, games_won, total_games) VALUES (%s, %s, %d, %d, %s, %d, %d, %d);\n",
