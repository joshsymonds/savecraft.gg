...
async function leaderboard(
  ...
  if (archetype) {
    ...
    const countResult = await db
      .prepare(
        "SELECT COUNT(*) as cnt FROM mtga_draft_ratings r JOIN magic_cards c ON c.front_face_name = r.card_name AND c.is_default = 1 AND c.rarity = ?2 AND c.type_line NOT LIKE 'Basic%Land%' WHERE r.set_code = ?1",
      )
      .bind(setCode, rarity)
      .first<{ cnt: number }>();
    total = countResult?.cnt ?? 0;
    ...
    const result = await db
      .prepare(
        `SELECT r.* FROM mtga_draft_ratings r JOIN magic_cards c ON c.front_face_name = r.card_name AND c.is_default = 1 AND c.rarity = ?2 AND c.type_line NOT LIKE 'Basic%Land%' WHERE r.set_code = ?1 ORDER BY r.${field} ${direction} LIMIT ?3 OFFSET ?4`,
      )
